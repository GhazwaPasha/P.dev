...
Blob drift keyframes, cross-fade page transitions, staggered card entrance, cross-fade avatar load-in.
*Status: not started. (The butterfly cursor this phase used to also cover was removed outright — see below — rather than polished.)*
...
5. **Bug fix (found while building the arc): butterfly cursor was eating all hover/click on Home.** React-three-fiber sets `pointer-events: auto` inline on its canvas wrapper by default, which combined with the cursor's `z-index: 9999` and 1:1 pointer tracking meant it silently sat on top of every element under the real mouse — nav icons, links, badges, all of it — and blocked their `:hover`/click. Fixed by forcing `pointerEvents: 'none'` on the `Canvas` (the component itself no longer exists — see below). This was blocking Phase 2 entirely and wasn't visible without inspecting `document.querySelectorAll(':hover')` directly — worth keeping in mind if any *other* future hover/click feature on Home mysteriously doesn't fire.
6. **Butterfly cursor removed.** The 3D butterfly cursor (`ButterflyCursor.tsx`/`ButterflyModel.tsx`, `public/models/butterfly.glb`) was dropped entirely per user request — Home now uses the default system cursor, same as About/Projects. `RootLayout.tsx` no longer sets `cursor: none` for Home, and `NavPill.tsx` no longer needs its `cursor: 'default'` override to win back over that. **Capabilities card** — added a compact glass card on the left of the Home hero (mirroring the identity card's placement on the right): a "What I do" heading plus a `LiquidGlassPillRow` of the same tech tags shown in About's "What I do" section, sourced from the same [content/about.ts](src/content/about.ts) `pillars` data so it can't drift out of sync. [Home.tsx](src/pages/Home.tsx), [CapabilitiesCard.tsx](src/components/hero/CapabilitiesCard.tsx).

## Recommended next step

Phases 1 and 2 are both done (see above). Phase 3 (blob drift keyframes, cross-fade page transitions, staggered card entrance, cross-fade avatar load-in) is next — it's the one phase that actually earns the "liquid" name, and it's the largest remaining chunk of untouched work.
